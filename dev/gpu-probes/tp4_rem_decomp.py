"""Remainder decomposition at 8 streams: how much is per-position sampler calls
(receive_logits/receive_sample) vs the rest of the accept/emit machinery."""
import os, time, json
os.environ["EXL3_NGRAM_STREAM"] = "1"
os.environ["EXL3_P2B_MOE"] = "1"

def main():
    import torch
    from exllamav3 import Config, Model, Cache, Tokenizer, ComboSampler, CacheLayer_fp16
    from exllamav3.generator import Generator, Job
    from exllamav3.generator.job import Job as JobCls
    import exllamav3.generator.generator as genmod

    config = Config.from_directory("/models/exl3-4.05bpw")
    model = Model.from_config(config)
    draft = Model.from_config(config, component = "mtp")
    cache = Cache(model, max_num_tokens = 8192, max_batch_size = 8, max_history = 8)
    draft_cache = Cache(draft, max_num_tokens = 8192, layer_type = CacheLayer_fp16)
    for p in draft.load_gen(use_per_device = [22, 22, 22, 22], verbose = False):
        pass
    for p in model.load_gen(device = None, use_per_device = [22, 22, 22, 22],
                            tensor_p = True, tp_output_device = 0,
                            tp_backend = "native", verbose = False):
        pass
    print("loaded", flush = True)

    tokenizer = Tokenizer.from_config(config)
    generator = Generator(model, cache, tokenizer,
                          draft_model = draft, draft_cache = draft_cache,
                          num_draft_tokens = 3, dynamic_draft_tokens = False,
                          record_draft_stats = True)
    sampler = ComboSampler(temperature = 0.6, top_k = 20, top_p = 0.95)

    T = {"rl": 0.0, "rs": 0.0, "rl_n": 0, "rs_n": 0, "rounds": 0}
    orig_rl = JobCls.receive_logits
    def rl(self, *a, **kw):
        t0 = time.perf_counter()
        r = orig_rl(self, *a, **kw)
        T["rl"] += time.perf_counter() - t0; T["rl_n"] += 1
        return r
    JobCls.receive_logits = rl
    orig_rs = JobCls.receive_sample
    def rs(self, *a, **kw):
        t0 = time.perf_counter()
        r = orig_rs(self, *a, **kw)
        T["rs"] += time.perf_counter() - t0; T["rs_n"] += 1
        return r
    JobCls.receive_sample = rs

    acc = {"draft": [], "verify_sync": [], "total": []}
    state = {"in_round": False, "draft_t": 0.0, "fwd_sync_t": 0.0}
    orig_dd = genmod.Generator.iterate_draftmodel_mtp_gen
    def dd(self, results):
        t0 = time.perf_counter()
        r = orig_dd(self, results)
        state["draft_t"] = time.perf_counter() - t0
        return r
    genmod.Generator.iterate_draftmodel_mtp_gen = dd
    orig_fwd = model.forward
    def fwd(*a, **kw):
        t0 = time.perf_counter()
        r = orig_fwd(*a, **kw)
        if state["in_round"]:
            state["fwd_sync_t"] = state.get("fwd_sync_t", 0.0) + (time.perf_counter() - t0)
            torch.cuda.synchronize()
            state["fwd_sync_t"] = state.get("fwd_sync_t", 0.0) + (time.perf_counter() - t0)
        return r
    model.forward = fwd
    orig_it = genmod.Generator.iterate
    def it(self):
        state["in_round"] = True
        state["fwd_sync_t"] = 0.0
        t0 = time.perf_counter()
        r = orig_it(self)
        dt = time.perf_counter() - t0
        state["in_round"] = False
        acc["draft"].append(state["draft_t"]); acc["verify_sync"].append(state["fwd_sync_t"])
        acc["total"].append(dt); T["rounds"] += 1
        return r
    genmod.Generator.iterate = it

    names = ["short", "small1", "healthy_long", "codeheavy"]
    prompts = {}
    for name in names:
        req = json.load(open(f"/host/{name}.json"))
        prompts[name] = tokenizer.encode(tokenizer.hf_render_chat_template(req["messages"]))

    def run_batch(tag):
        jobs = []
        for i in range(8):
            jobs.append(Job(input_ids = prompts[names[i % 4]].clone(),
                            max_new_tokens = 300, sampler = sampler,
                            identifier = f"{tag}-{i}"))
        n0 = T["rounds"]
        t0 = time.perf_counter()
        for job in jobs:
            generator.enqueue(job)
        while generator.num_remaining_jobs():
            generator.iterate()
        dt = time.perf_counter() - t0
        n = T["rounds"] - n0
        d = sum(acc["draft"][n0:]) / n * 1000
        v = sum(acc["verify_sync"][n0:]) / n * 1000
        t = sum(acc["total"][n0:]) / n * 1000
        rl = T["rl"] / T["rounds"] * 1000
        rs = T["rs"] / T["rounds"] * 1000
        rem = t - d - v
        print(f"[{tag}] round {t:.1f}ms | verify {v:.1f} draft {d:.1f} remainder {rem:.1f} "
              f"| receive_logits {rl:.1f}ms ({T['rl_n']//T['rounds']}/rnd) "
              f"receive_sample {rs:.1f}ms ({T['rs_n']//T['rounds']}/rnd) "
              f"| other-remainder {rem-rl-rs:.1f}ms", flush = True)

    run_batch("warm")
    run_batch("meas")
    print("REMAINDER DECOMP DONE", flush = True)

if __name__ == "__main__":
    main()
