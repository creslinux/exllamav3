...
class CacheLayer_qsa_quant(CacheLayer_quant):
    """
    Quantised KV cache layer with the QSA indexer's fp16 side planes. The indexer planes (raw
    per-token keys, pooled per-block keys) deliberately STAY fp16 so block selection is
    bit-identical to the fp16 layer; only the attention K/V are quantised (k_bits/v_bits).
    """
