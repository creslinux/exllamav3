...
class BCAttn:
    ...
    def _configure_qsa(self, bsz: int, q_len: int, regime: int):
        ...
        if regime == 1:
            ...
            if self.quant:
                k_sp_split = _compile_kernel(dev, _qsa_sparse_split_kernel_q,
                    {"q": "*fp16", "k_cache": "*i32", "v_cache": "*i32", "block_table": "*i32",
            ...
            k_sp_combine = _compile_kernel(dev, _paged_attn_decode_combine_kernel,
                ...
                dict(QCV = self.v_bits if self.quant else 0, HAS_SINKS = False, q_len = 1,
...
def build_bc_attn(module, layer):
    ...
    if not (
        ...
        # A QSA module needs the fp16 side planes on this layer (fp16 or quantised K/V)
        (qsa_idx is None or (
            isinstance(layer, (CacheLayer_qsa, CacheLayer_qsa_quant)) and layer.raw_k is not None and
